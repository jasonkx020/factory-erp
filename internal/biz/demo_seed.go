...
func seedDemoCRM(db *sql.DB, today, now string) {
	_, _ = db.Exec(`INSERT OR IGNORE INTO crm_customer(id, code, name, short_name, contact_name, mobile, address, level, source, status, owner_user_id, is_public_sea, is_locked, is_hidden, settle_method, payment_days, credit_limit, remark) VALUES
 ...
 (12, 1, 1, 1, 'wechat', ?, '微信发送报价单与样品照片', datetime('now','+5 day'))`, now, now)
	...
	_, _ = db.Exec(`INSERT OR IGNORE INTO crm_lead_assign(id, customer_id, from_user_id, to_user_id, assigned_at, lock_flag, remark) VALUES
 (1, 3, NULL, 1, ?, 0, '公海领取演示'),
 (2, 11, 1, 1, ?, 1, '资源分配演示')`, now, now)

	_, _ = db.Exec(`INSERT OR IGNORE INTO crm_lead_release_log(id, customer_id, released_at, reason, to_public_sea, from_user_id, operator_user_id) VALUES
 ...
 (12, 1, 'customer', 12, datetime('now','+3 day'), '解锁北海客户后回访', 'pending')`)
	...
	_, _ = db.Exec(`INSERT OR IGNORE INTO crm_customer_import_batch(id, file_name, imported_at, success_count, fail_count, fail_detail_json, created_by) VALUES
 (1, 'customers_demo.xlsx', ?, 18, 2, '[{"row":3,"err":"手机号重复"}]', 1)`, now)
}

func seedDemoSales(db *sql.DB, today, now string) {
	_, _ = db.Exec(`INSERT OR IGNORE INTO sl_inquiry(doc_no, customer_id, status, owner_user_id, remark, created_at) VALUES
 ...
 ('DEMO-INQ-002', 11, 'pending', 1, '待询价审批', ?)`, now, now)
	...
	_, _ = db.Exec(`INSERT OR IGNORE INTO sl_quote_history(customer_id, product_id, price, quoted_at, inquiry_id) VALUES
 (1, 3, 6.8, ?, ?), (11, 3, 6.5, ?, ?)`, today, nullIf0(inq1), today, nullIf0(inq2))

	_, _ = db.Exec(`INSERT OR IGNORE INTO sl_contract(doc_no, customer_id, title, amount, status, signed_at, remark) VALUES
 ...
 ('DEMO-CT-002', 11, '桂林零食供货合同', 86000, 'draft', ?, '草稿合同')`, today, today)
	...
	_, _ = db.Exec(`INSERT OR IGNORE INTO sl_sales_order(doc_no, customer_id, owner_user_id, status, source, contract_id, warehouse_id, total_amount, remark, created_by) VALUES
 ...
 ('DEMO-SO-003', 2, 1, 'shipped', 'manual', NULL, 3, 9800, '已发货复购样例', 1)`, nullIf0(ct1))
	...
	_, _ = db.Exec(`INSERT OR IGNORE INTO sl_pre_shipment(doc_no, order_id, plan_ship_date, status, reserved, warehouse_id, remark) VALUES
 ('DEMO-PS-001', ?, ?, 'approved', 1, 3, '预发货演示')`, nullIf0(so1), today)
	...
	_, _ = db.Exec(`INSERT OR IGNORE INTO sl_delivery_approval(doc_no, order_id, pre_shipment_id, status, warehouse_id, logistics_no, remark) VALUES
 ...
 ('DEMO-DA-002', ?, NULL, 'shipped', 3, 'YD99887766', '已发运')`, nullIf0(so1), nullIf0(ps1), nullIf0(so3))
	...
	_, _ = db.Exec(`INSERT OR IGNORE INTO sl_sales_bom(doc_no, order_id, product_id, name, status) VALUES
 ('DEMO-SBOM-001', ?, 3, '袋装木薯丁销售BOM', 'active')`, nullIf0(so1))
	...
	_, _ = db.Exec(`INSERT OR IGNORE INTO sl_quote_calculator_result(customer_id, product_id, qty, base_cost, margin_rate, quote_price, payload_json) VALUES
 (1, 3, 1000, 4.2, 0.25, 5.25, '{"freight":0.3,"pack":0.2}')`)

	_, _ = db.Exec(`INSERT OR IGNORE INTO sl_print_log(doc_type, doc_id, doc_no, template_code, printed_by, printed_at) VALUES
 ('sales_order', ?, 'DEMO-SO-001', 'SO_A4', 1, ?)`, nullIf0(so1), now)

	_, _ = db.Exec(`INSERT OR IGNORE INTO sl_outbound_settle(doc_no, biz_date, product_id, product_name, plate_no, driver_name, qty, weight, unit_price, goods_amount, amount, status, remark) VALUES
...
func seedDemoPurchase(db *sql.DB, today, now string) {
	_, _ = db.Exec(`INSERT OR IGNORE INTO pur_purchase_request(doc_no, applicant_id, title, qty, status, need_date, remark) VALUES
 ('DEMO-PR-001', 1, '鲜木薯补货申请', 10000, 'approved', ?, '采购申请演示')`, today)
	...
	_, _ = db.Exec(`INSERT OR IGNORE INTO pur_purchase_plan(doc_no, status, plan_date, remark) VALUES
 ('DEMO-PP-001', 'approved', ?, '月度采购计划演示')`, today)
	...
	_, _ = db.Exec(`INSERT OR IGNORE INTO pur_purchase_inbound(doc_no, supplier_id, warehouse_id, status, biz_date, remark) VALUES
 ...
 ('DEMO-PI-002', 2, 1, 'draft', ?, '包装袋待入')`, today, today)
	...
	_, _ = db.Exec(`INSERT OR IGNORE INTO pur_incoming_qc(doc_no, inbound_id, supplier_id, product_id, qty_check, qty_pass, qty_fail, result, status, remark) VALUES
 ('DEMO-IQC-001', ?, 1, 1, 5000, 4850, 150, 'pass', 'done', '来料质检演示')`, nullIf0(pi1))

	_, _ = db.Exec(`INSERT OR IGNORE INTO pur_purchase_return(doc_no, supplier_id, inbound_id, warehouse_id, status, remark) VALUES
 ('DEMO-PRT-001', 1, ?, 1, 'draft', '采购退货演示')`, nullIf0(pi1))
	...
	_, _ = db.Exec(`INSERT OR IGNORE INTO pur_purchase_task(doc_no, assignee_id, product_id, qty, status, due_date) VALUES
...
func seedDemoProduction(db *sql.DB, today, now string) {
	_, _ = db.Exec(`INSERT OR IGNORE INTO pd_production_task(doc_no, source_type, status, plan_start, plan_end, routing_id, workshop_id, owner_user_id, remark) VALUES
 ...
 ('DEMO-TK-002', 'manual', 'pending', ?, date('now','+5 day'), 1, 1, 1, '待释放任务')`, today, today)
	...
	_, _ = db.Exec(`INSERT OR IGNORE INTO pd_work_order(doc_no, task_id, process_id, routing_step_id, status, plan_qty) VALUES
 ...
 ('DEMO-WO-002', ?, 4, 6, 'pending', 2000)`, nullIf0(tk1), nullIf0(tk1))
	...
	_, _ = db.Exec(`INSERT OR IGNORE INTO pd_dispatch(doc_no, work_order_id, dispatch_type, worker_id, plan_qty, status, dispatched_at, created_by) VALUES
 ...
 ('DEMO-DP-002', ?, 'flex', 3, 500, 'reassigned', ?, 1)`, nullIf0(wo1), now, nullIf0(wo1), now)
	...
	_, _ = db.Exec(`INSERT OR IGNORE INTO pd_report_work(doc_no, dispatch_id, work_order_id, process_id, worker_id, report_type, qty, weight, status, reported_at, scan_code) VALUES
 ...
 ('DEMO-RW-002', ?, ?, 1, 3, 'output', 500, 500, 'submitted', ?, 'SCAN-DEMO-02')`,
	...
	_, _ = db.Exec(`INSERT OR IGNORE INTO pd_qc_order(doc_no, qc_type, product_id, process_id, qty, result, status) VALUES
 ...
 ('DEMO-QC-002', 'final', 3, 11, 500, 'pending', 'pending')`)
	...
	_, _ = db.Exec(`INSERT OR IGNORE INTO pd_rework_order(doc_no, source_qc_id, task_id, process_id, qty, status, remark) VALUES
 ('DEMO-RWK-001', ?, ?, 6, 50, 'doing', '袋装破损返修演示')`, nullIf0(qc1), nullIf0(tk1))

	_, _ = db.Exec(`INSERT OR IGNORE INTO pd_scrap_record(doc_no, task_id, process_id, product_id, qty, disposition, status) VALUES
 ('DEMO-SCR-001', ?, 1, 1, 120, 'waste', 'posted')`, nullIf0(tk1))

	_, _ = db.Exec(`INSERT OR IGNORE INTO pd_drawing_link(drawing_code, drawing_name, task_id, process_id, file_url, status) VALUES
 ('DWG-FG-001', '袋装木薯丁图纸', ?, 11, '/files/demo/fg_diced.pdf', 'active')`, nullIf0(tk1))

	_, _ = db.Exec(`INSERT OR IGNORE INTO pd_task_merge(merge_no, title, status, result_task_id, remark) VALUES
 ('DEMO-MG-001', '多单整合演示', 'done', ?, '合并 DEMO-TK-001/002')`, nullIf0(tk1))
	...
	_, _ = db.Exec(`INSERT OR IGNORE INTO pd_outsource_order(doc_no, supplier_id, process_id, product_id, qty, status, remark) VALUES
 ('DEMO-OSR-001', 2, 6, 3, 300, 'doing', '委外装袋演示')`)

	_, _ = db.Exec(`INSERT OR IGNORE INTO pd_consignment_order(doc_no, customer_id, product_id, qty, status, progress, remark) VALUES
 ('DEMO-CSG-001', 1, 3, 500, 'doing', '半成品入库', '受托加工演示')`)

	_, _ = db.Exec(`INSERT OR IGNORE INTO pd_mrp_run(run_no, run_at, status, params_json, remark) VALUES
 ('DEMO-MRP-001', ?, 'done', '{"horizon_days":7}', 'MRP演示')`, now)
	...
	_, _ = db.Exec(`INSERT OR IGNORE INTO pd_cost_hide_policy(role_id, name, field_scope, is_enabled) VALUES
 (1, '管理员可见成本', '["cost","margin"]', 1)`)

	_, _ = db.Exec(`INSERT OR IGNORE INTO pd_piece_issue_sheet(doc_no, biz_date, status, remark) VALUES
...
func seedDemoInventory(db *sql.DB, today, now string) {
	_, _ = db.Exec(`INSERT OR IGNORE INTO inv_stock_txn(doc_no, doc_type, biz_date, status, warehouse_id, remark, posted_at, created_by) VALUES
 ...
 ('DEMO-ST-TR-001', 'transfer', ?, 'draft', 1, '调拨草稿', NULL, 1)`, today, now, today, now, today)
	...
	_, _ = db.Exec(`INSERT OR IGNORE INTO inv_reservation(warehouse_id, product_id, batch_no, qty, source_doc_type, source_doc_id, status)
	...
	_, _ = db.Exec(`INSERT OR IGNORE INTO inv_inbound_qc(doc_no, stock_txn_id, product_id, qty_check, qty_pass, qty_fail, result, status, remark) VALUES
 ('DEMO-IIQC-001', ?, 1, 5000, 4850, 150, 'pass', 'done', '入库质检演示')`, nullIf0(inID))

	_, _ = db.Exec(`INSERT OR IGNORE INTO inv_stocktake(doc_no, stocktake_type, warehouse_id, workshop_id, biz_date, status, remark) VALUES
 ...
 ('DEMO-TKP-001', 'workshop', NULL, 1, ?, 'draft', '车间盘点演示')`, today, today)
	...
	_, _ = db.Exec(`INSERT OR IGNORE INTO inv_transfer(doc_no, from_warehouse_id, to_warehouse_id, biz_date, status, remark) VALUES
 ('DEMO-TF-001', 1, 2, ?, 'posted', '原料转半成品库')`, today)
	...
	_, _ = db.Exec(`INSERT OR IGNORE INTO inv_consume(doc_no, warehouse_id, biz_date, status, remark) VALUES
 ('DEMO-CS-001', 1, ?, 'posted', '生产耗用演示')`, today)
	...
	_, _ = db.Exec(`INSERT OR IGNORE INTO inv_assemble_split(doc_no, biz_type, warehouse_id, status, remark) VALUES
 ('DEMO-AS-001', 'assemble', 3, 'draft', '组装拆分演示')`)
	...
	_, _ = db.Exec(`INSERT OR IGNORE INTO inv_price_adjust(doc_no, product_id, old_price, new_price, effective_at, status, remark) VALUES
 ('DEMO-PA-001', 3, 7.0, 7.2, ?, 'posted', '调价演示')`, today)
	...
	so3 := demoID(db, `SELECT id FROM sl_sales_order WHERE doc_no='DEMO-SO-003'`)
	_, _ = db.Exec(`INSERT OR IGNORE INTO inv_sales_peel_return(doc_no, sales_order_id, product_id, peel_qty, weight, warehouse_id, status, remark) VALUES
 ('DEMO-PEEL-001', ?, 3, 20, 20, 3, 'posted', '销售退皮演示')`, nullIf0(so3))

	_, _ = db.Exec(`INSERT OR IGNORE INTO inv_material_to_payable(doc_no, supplier_id, product_id, qty, amount, status, remark) VALUES
 ('DEMO-MTP-001', 1, 1, 150, 277.5, 'draft', '物料转应付演示')`)

	_, _ = db.Exec(`INSERT OR IGNORE INTO inv_box_code(code, product_id, warehouse_id, batch_no, qty, weight, status) VALUES
 ('BX-SEMI-DEMO', 2, 2, 'B0803', 500, 500, 'open'),
 ('BX-FG-DEMO', 3, 3, 'FG-SEED', 200, 200, 'open')`)
}

func seedDemoProduct(db *sql.DB) {
	_, _ = db.Exec(`INSERT OR IGNORE INTO prd_product(id, code, name, product_type, cost_price, sale_price, status, spec_text) VALUES
 (11, 'FG-CHIPS', '木薯脆片', 'finished', 5.5, 9.8, 'active', '200g/袋'),
 (12, 'PK-BAG', '包装袋', 'pack', 0.3, NULL, 'active', '透明袋')`)
	_, _ = db.Exec(`INSERT OR IGNORE INTO prd_product_unit(product_id, unit_name, is_base, factor_to_base) VALUES
 (11, '袋', 1, 1), (12, '个', 1, 1)`)
	_, _ = db.Exec(`INSERT OR IGNORE INTO prd_product_spec(id, product_id, spec_code, routing_id, remark, status) VALUES
 (1, 3, 'SPEC-DICED-STD', 1, '标准丁规格绑定工艺', 'active'),
 (2, 11, 'SPEC-CHIPS-A', 1, '脆片规格演示', 'active')`)
	_, _ = db.Exec(`INSERT OR IGNORE INTO prd_product_app_sort(product_id, channel, sort_no, is_visible) VALUES
...
func seedDemoAsset(db *sql.DB, today string) {
	...
	_, _ = db.Exec(`INSERT OR IGNORE INTO ast_fixed_asset(code, name, category_id, dept_id, dept_name, location_text, original_value, net_value, status, purchase_date, useful_life_months, residual_rate, remark) VALUES
 ...
 ('FA-DEMO-003', '叉车01', ?, 2, '仓储部', '原料仓门口', 45000, 30000, 'active', '2022-01-10', 60, 0.05, '运输工具')`, catID, catID, catID)
	...
	_, _ = db.Exec(`INSERT OR IGNORE INTO ast_asset_transfer(doc_no, asset_id, from_dept_id, to_dept_id, from_dept_name, to_dept_name, from_location, to_location, status, remark, transferred_at) VALUES
...
func seedDemoFinance(db *sql.DB, today, now, period string) {
	...
	_, _ = db.Exec(`INSERT OR IGNORE INTO fin_voucher(doc_no, period, biz_date, status, summary) VALUES
 ...
 ('DEMO-V-002', ?, ?, 'draft', '采购付款草稿')`, period, today, period, today)
	...
	so1 := demoID(db, `SELECT id FROM sl_sales_order WHERE doc_no='DEMO-SO-001'`)
	_, _ = db.Exec(`INSERT OR IGNORE INTO fin_ledger_entry(doc_no, account_id, subject_id, direction, amount, biz_date, counterparty, source_doc_type, source_doc_id, remark) VALUES
 ...
 ('DEMO-LE-IN-002', ?, ?, 'in', 5000, ?, '柳州餐饮连锁', 'recognition', 0, '认款')`,
	...
	_, _ = db.Exec(`INSERT OR IGNORE INTO fin_invoice(invoice_no, direction, counterparty_id, counterparty_name, amount, tax, status, biz_date) VALUES
 ('DEMO-INV-OUT-001', 'out', 1, '南宁食品批发部', 13600, 1768, 'issued', ?),
 ('DEMO-INV-IN-001', 'in', 1, '广西木薯原料合作社', 9250, 1202.5, 'received', ?)`, today, today)

	_, _ = db.Exec(`INSERT OR IGNORE INTO fin_receipt_writeoff(doc_no, customer_id, amount, fund_account_id, status, received_at) VALUES
 ('DEMO-WO-001', 1, 5000, ?, 'posted', ?)`, acc1, today)
	_, _ = db.Exec(`INSERT OR IGNORE INTO fin_payment_recognition(doc_no, customer_id, amount, fund_account_id, status, remark) VALUES
 ('DEMO-RG-001', 2, 3000, ?, 'posted', '销售认款演示')`, acc1)
	_, _ = db.Exec(`INSERT OR IGNORE INTO fin_prepay_prepaid(doc_no, party_type, party_id, direction, amount, balance, status) VALUES
 ('DEMO-PPY-001', 'customer', 1, 'in', 10000, 5000, 'open'),
 ('DEMO-PPY-002', 'supplier', 1, 'out', 8000, 8000, 'open')`)
	_, _ = db.Exec(`INSERT OR IGNORE INTO fin_fx_settlement(doc_no, currency, amount_fx, rate, amount_local, fund_account_id, status) VALUES
 ('DEMO-FX-001', 'USD', 1000, 7.2, 7200, ?, 'posted'),
 ('DEMO-FX-002', 'USD', 500, 7.15, 3575, ?, 'draft')`, acc1, acc1)
	_, _ = db.Exec(`INSERT OR IGNORE INTO fin_cost_allocation(doc_no, source_amount, alloc_json, status) VALUES
 ('DEMO-CA-001', 3000, '{"workshop":1,"rate":1}', 'posted')`)
	_, _ = db.Exec(`INSERT OR IGNORE INTO fin_cashier_reconcile(doc_no, fund_account_id, biz_date, book_balance, actual_balance, status, remark) VALUES
 ('DEMO-CR-001', ?, ?, 50000, 49850, 'draft', '出纳对账演示')`, acc1, today)
	_, _ = db.Exec(`INSERT OR IGNORE INTO fin_cost_accounting(doc_no, period, task_id, product_id, material_cost, labor_cost, overhead, total_cost, status) VALUES
 ('DEMO-COST-001', ?, ?, 3, 8000, 1200, 800, 10000, 'posted')`, period, demoID(db, `SELECT id FROM pd_production_task WHERE doc_no='DEMO-TK-001'`))
	_, _ = db.Exec(`INSERT OR IGNORE INTO fin_sales_return_finance(doc_no, order_id, amount, status) VALUES
 ('DEMO-SRF-001', ?, 140, 'draft')`, demoID(db, `SELECT id FROM sl_sales_order WHERE doc_no='DEMO-SO-003'`))
	_, _ = db.Exec(`INSERT OR IGNORE INTO fin_arap_adjust(doc_no, party_type, party_id, amount, direction, status, remark) VALUES
 ('DEMO-ARAP-001', 'customer', 1, 200, 'increase', 'posted', '往来调整演示'),
 ('DEMO-ARAP-002', 'supplier', 1, 150, 'decrease', 'draft', '应付调减')`)
	_, _ = db.Exec(`INSERT OR IGNORE INTO fin_month_close(year, month, status) VALUES (?, ?, 'open')`, time.Now().Year(), int(time.Now().Month()))
	_, _ = db.Exec(`INSERT OR IGNORE INTO fin_miniprogram_bill(bill_no, channel, amount, status, order_id) VALUES
 ('DEMO-MP-001', 'wechat', 6500, 'paid', ?)`, demoID(db, `SELECT id FROM sl_sales_order WHERE doc_no='DEMO-SO-002'`))
	_, _ = db.Exec(`INSERT OR IGNORE INTO fin_fund_transfer(doc_no, from_account_id, to_account_id, amount, status, remark) VALUES
 ('DEMO-FT-001', ?, ?, 10000, 'posted', '资金调拨演示')`, acc1, acc2)
	_, _ = db.Exec(`INSERT OR IGNORE INTO fin_statement_cache(code, period, title, content_json) VALUES
...
func seedDemoHR(db *sql.DB, today, now, period string) {
	_, _ = db.Exec(`INSERT OR IGNORE INTO hr_employee(emp_no, name, org_id, dept_id, workshop_id, emp_type, status) VALUES
 ...
 ('E1102', '演示销售', 1, 1, NULL, 'sales', 'active')`)
	...
	_, _ = db.Exec(`INSERT OR IGNORE INTO hr_leave_request(doc_no, employee_id, leave_type, start_at, end_at, status, remark) VALUES
 ...
 ('DEMO-LV-002', 3, 'sick', date('now','-2 day') || ' 09:00:00', date('now','-2 day') || ' 18:00:00', 'approved', '已批病假')`, today, today)
	...
	_, _ = db.Exec(`INSERT OR IGNORE INTO hr_overtime_patch(doc_no, employee_id, biz_type, biz_date, minutes, status, remark) VALUES
 ...
 ('DEMO-OT-002', 3, 'patch', ?, 30, 'approved', '漏打卡补卡')`, today, today)
	...
	_, _ = db.Exec(`INSERT OR IGNORE INTO hr_attendance_month_stat(employee_id, year, month, work_days, late_times, ot_hours, leave_days) VALUES
 (2, ?, ?, 21, 1, 6, 1),
 (3, ?, ?, 20, 3, 2, 0)`, y, m, y, m)
 ...
 (?,2,'小刀',1,1)`, demoTI2)
	...
	_, _ = db.Exec(`INSERT OR IGNORE INTO hr_attendance_perf_summary(employee_id, period, attendance_score, perf_score, summary_json) VALUES
...
func seedDemoPayroll(db *sql.DB, today, now string) {
	...
	_, _ = db.Exec(`INSERT OR IGNORE INTO pay_payroll_sheet(doc_no, period_year, period_month, status, workshop_id, calc_at, remark, created_by) VALUES
...
func seedDemoApproval(db *sql.DB) {
	_, _ = db.Exec(`INSERT OR IGNORE INTO appr_expense_request(doc_no, applicant_id, amount, category, status, remark) VALUES
 ('DEMO-ER-001', 1, 860, '差旅费', 'submitted', '费用申请已提交'),
 ('DEMO-ER-002', 1, 320, '办公费用', 'draft', '费用申请草稿')`)
	_, _ = db.Exec(`INSERT OR IGNORE INTO appr_affair_request(doc_no, applicant_id, title, content, status, remark) VALUES
 ('DEMO-AF-001', 1, '申请更换车间照明', '一车间灯管老化需更换', 'submitted', '事务申请演示')`)
	...
	so1 := demoID(db, `SELECT id FROM sl_sales_order WHERE doc_no='DEMO-SO-001'`)
	_, _ = db.Exec(`INSERT OR IGNORE INTO appr_queue(category, doc_no, title, biz_type, biz_id, amount, applicant_id, status, remark) VALUES
 ...
 ('doc_review', 'DEMO-SO-001', '销售订单审核 DEMO-SO-001', 'sales_order', ?, 13600, 1, 'pending', '单据审核演示')`, nullIf0(er1), nullIf0(so1))
	_, _ = db.Exec(`INSERT OR IGNORE INTO appr_task(doc_type, doc_id, assignee_user_id, status, title, doc_no, amount, applicant_id, remark) VALUES
...
func seedDemoSystem(db *sql.DB, today, now, period string) {
	_, _ = db.Exec(`INSERT OR IGNORE INTO sys_print_template(id, code, name, doc_type, content, status) VALUES
 (1, 'SO_A4', '销售订单A4', 'sales_order', '<h1>销售订单</h1>', 'active'),
 (2, 'PO_A4', '采购入库单', 'purchase_inbound', '<h1>采购入库</h1>', 'active')`)

	_, _ = db.Exec(`INSERT OR IGNORE INTO sys_formula(id, code, name, scope, expression, remark, status) VALUES
 (1, 'MARGIN', '毛利率', 'sales', '(sale-cost)/sale', '报价计算', 'active')`)

	_, _ = db.Exec(`INSERT OR IGNORE INTO sys_carrier(id, code, name, contact, phone, status) VALUES
 (1, 'SF', '顺丰速运', '客服', '95338', 'active'),
 (2, 'YD', '韵达快递', '客服', '95546', 'active')`)

	_, _ = db.Exec(`INSERT OR IGNORE INTO sys_approval_flow(id, code, name, doc_type, status) VALUES
 (1, 'FLOW-SO', '销售订单审批流', 'sales_order', 'active'),
 (2, 'FLOW-PO', '采购入库审批流', 'purchase_inbound', 'active')`)
	_, _ = db.Exec(`INSERT OR IGNORE INTO sys_approval_flow_node(id, flow_id, seq_no, node_name, approver_role, require_all) VALUES
 (1, 1, 1, '销售主管', 'sales_manager', 0),
 (2, 1, 2, '财务审核', 'finance', 0),
 (3, 2, 1, '采购主管', 'purchase_manager', 0)`)

	_, _ = db.Exec(`INSERT OR IGNORE INTO sys_personnel_transfer(id, doc_no, employee_id, from_dept_id, to_dept_id, reason, status, effective_date) VALUES
 (1, 'DEMO-PTF-001', 3, 1, 1, '岗位轮换演示', 'draft', ?)`, today)

	_, _ = db.Exec(`INSERT OR IGNORE INTO sys_batch_price_job(id, doc_no, target_type, adjust_type, adjust_value, status, result_msg) VALUES
 (1, 'DEMO-BP-001', 'product', 'percent', 0.05, 'done', '已调价3个SKU')`)

	_, _ = db.Exec(`INSERT OR IGNORE INTO sys_batch_payroll_job(id, doc_no, period_ym, workshop_id, status, result_msg) VALUES
 (1, 'DEMO-BPAY-001', ?, 1, 'done', '已核算21人')`, period)

	_, _ = db.Exec(`INSERT OR IGNORE INTO sys_reminder(id, title, content, remind_at, target_user_id, status) VALUES
 (1, '月结提醒', '请完成本月财务月结', datetime('now','+2 day'), 1, 'open')`)

	_, _ = db.Exec(`INSERT OR IGNORE INTO sys_announcement(id, title, content, status, published_at, created_by) VALUES
 (1, '安全生产月通知', '全厂开展安全检查，请各部门配合。', 'published', ?, 1)`, now)

	_, _ = db.Exec(`INSERT OR IGNORE INTO sys_memo(id, title, content, owner_id, status) VALUES
 (1, '系统演示备忘', '客户验收走查清单', 1, 'open')`)

	_, _ = db.Exec(`INSERT OR IGNORE INTO sys_document(id, code, title, category, content, status) VALUES
 (1, 'DOC-DEMO-01', '木薯加工工艺说明', '工艺', '演示文档内容', 'active')`)

	_, _ = db.Exec(`INSERT OR IGNORE INTO sys_drawing(id, code, title, product_id, version_no, file_url, status) VALUES
 (1, 'DWG-DEMO-01', '袋装木薯丁包装图', 3, 'V1', '/files/demo/pack.png', 'active')`)

	_, _ = db.Exec(`INSERT OR IGNORE INTO sys_knowledge(id, code, title, category, content, status) VALUES
 (1, 'KB-DEMO-01', '去皮工序作业指导', '生产', '演示知识库条目', 'active')`)

	_, _ = db.Exec(`INSERT OR IGNORE INTO sys_course(id, code, title, category, content, duration_min, status) VALUES
 (1, 'CRS-DEMO-01', '新员工入职安全培训', '学堂', '演示课程', 45, 'active')`)

	_, _ = db.Exec(`INSERT OR IGNORE INTO sys_data_repair(id, doc_no, target_type, target_id, action, reason, status, applied_at, created_by) VALUES
...
func seedDemoReport(db *sql.DB, today, now string) {
	...
	_, _ = db.Exec(`INSERT OR IGNORE INTO sys_logistics_track(id, track_no, carrier_id, order_id, status, location, updated_at) VALUES
 ...
 (2, 'YD99887766', ?, 3, 'delivered', '柳州已签收', ?)`, carrierID, now, carrierID, now)
	...
	_, _ = db.Exec(`INSERT OR IGNORE INTO rpt_report_snapshot(report_code, biz_date, payload_json) VALUES
 ('daily', ?, ?)`, today, payload)
	_, _ = db.Exec(`INSERT OR IGNORE INTO rpt_report_snapshot(report_code, biz_date, payload_json) VALUES
