...
func EnsureClosedLoopSchema(db *sql.DB) {
  ...
  status TEXT NOT NULL DEFAULT 'active'
	...
	alters := []string{
		...
		`ALTER TABLE pd_report_work ADD COLUMN operator_user_id INTEGER`,
		`ALTER TABLE pd_report_work ADD COLUMN operator_employee_id INTEGER`,
		`ALTER TABLE pd_report_work ADD COLUMN created_by INTEGER`,
 ...
 ('fg_in', '成品入库', 30, 'active', NULL, '成品入库过磅')`)
  ...
  used_at TEXT
	...
	_, _ = db.Exec(`ALTER TABLE pur_trace_batch_code ADD COLUMN reserved_by INTEGER`)
	_, _ = db.Exec(`ALTER TABLE pur_trace_batch_code ADD COLUMN reserved_at TEXT`)
}
...
func seedInboundProductRoutings(db *sql.DB) {
	_, _ = db.Exec(`INSERT OR IGNORE INTO prd_product(id, code, name, product_type, cost_price, sale_price, status) VALUES
...
func rebuildProductionFlowGraph(db *sql.DB) {
	...
	_, _ = db.Exec(`UPDATE pd_flow_graph SET name=?, graph_json=?, version_no='V1-fixed', updated_at=datetime('now')
	...
	log.Printf("automation: repaired RT-CASSAVA flow graph Chinese labels")
}
