...
func (s *Services) handleScan(c *gin.Context, resolveOnly bool) bool {
	...
	res, err := s.DB.Exec(`INSERT INTO pd_report_work(doc_no, dispatch_id, work_order_id, process_id, worker_id, qty, weight, qty_net, status, reported_at, scan_code)
		VALUES(?,?,?,?,?,?,?,?,'submitted',?,?)`, docNo, nullIf0(dispatchID), nullIf0(woID), processID, workerID, qty, qty, qty, now, box)
	if err != nil {
	...
	body = map[string]interface{}{
		...
		"process_id": processID, "worker_id": workerID, "qty": qty, "qty_net": qty,
	...
	flow := s.AfterReportWork(c, rid, processID, workerID, qty, box, taskID, woID, stepID)
	for k, v := range flow {
		body[k] = v
	}
