...
export const FIN_HINT: Record<string, string> = {
  ...
  prepays: '预收客户或预付供应商，登记即入账。',
  'cost-accountings': '按期间汇入供应商已付货款与计件日结，生成可审计成本单。',
  ...
  funds: '现金/银行账户余额与内部调拨；供应商付款从账户扣减。',
  ledger: '资金进出流水（含供应商货款支付）；可手工登记其他收支。',
  payables: '待付供应商结算单：选资金账户、填转账单号与回单后支付关单。开启在线支付后请到「在线支付审批」。',
  'payment-orders': '在线支付单：财务审批 →（可选）总经理审批 → 自动出款；未配置支付宝时走模拟通道。',
  statements: '由已过账凭证生成三大表；无凭证时回退流水。',
  'cost-traces': '成本核算来源明细（供应商货款/计件/领料），可跳转对账。',
  'month-closes': '结转后该月禁止流水、核单、调拨、供应商付款。',
}
