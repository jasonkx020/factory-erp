	...
	return []ticketCategorySeed{
		{
			...
			Fields: []FormFieldDef{
				...
				fieldSelect("channel", "过磅渠道", true, "厂内秤", "外磅单"),
				fieldNum("gross_weight", "入场重量", true, "kg"),
		...
		},
		{
			...
			Fields: []FormFieldDef{
				fieldText("batch_no", "溯源批号", true),
				fieldText("variety", "品种/产品", true),
				fieldNum("net_weight", "重量", true, "kg"),
				fieldNum("bag_qty", "袋数", false, ""),
				fieldSelect("cold_store_type", "冷库类型", true, "保鲜库", "半成品库", "成品库"),
				fieldText("origin", "产地地址", false),
				fieldText("remark", "备注", false),
			},
		},
		{
