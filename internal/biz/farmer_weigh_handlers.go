...
func (s *Services) listFarmers(c *gin.Context) bool {
	...
	where := `WHERE COALESCE(is_deleted,0)=0 AND party_kind='person'`
	args := []interface{}{}
	searching := idQ != "" || mobileQ != "" || nameQ != "" || kw != ""
	if searching {
		where += ` AND status NOT IN ('eliminated','blacklist')`
	}
	if idQ != "" {
		where += ` AND id=?`
	...
	if kw != "" {
		where += ` AND (name LIKE ? OR mobile LIKE ? OR code LIKE ? OR COALESCE(origin,'') LIKE ?)`
		like := "%" + kw + "%"
		args = append(args, like, like, like, like)
	}
	var total int
	_ = s.DB.QueryRow(`SELECT COUNT(1) FROM pur_supplier `+where, args...).Scan(&total)
	args = append(args, pageSize, (pageNum-1)*pageSize)
	rows, err := s.DB.Query(`SELECT id, code, name, COALESCE(mobile,''), COALESCE(origin,''),
		COALESCE(trace_code_prefix,''), status, COALESCE(remark,''), created_at, COALESCE(default_unit_price,0)
		FROM pur_supplier `+where+` ORDER BY id DESC LIMIT ? OFFSET ?`, args...)
	if err != nil {
	...
	for rows.Next() {
		var id int64
		var code, name, mobile, origin, prefix, status, remark, created string
		var price float64
		_ = rows.Scan(&id, &code, &name, &mobile, &origin, &prefix, &status, &remark, &created, &price)
		list = append(list, gin.H{
			"id": id, "code": code, "name": name, "mobile": mobile, "origin": origin,
			"trace_code": prefix, "trace_code_prefix": prefix, "status": status, "remark": remark, "created_at": created,
			"default_unit_price": price, "party_kind": "person",
		})
	}
...
func (s *Services) createFarmer(c *gin.Context) bool {
	...
	status := strOrDef(body["status"], "qualified")
	if status == "active" {
		status = "qualified"
	}
	...
	res, err := s.DB.Exec(`INSERT INTO pur_supplier(code, name, party_kind, supplier_type, status, mobile, origin, trace_code_prefix, remark, default_unit_price)
		VALUES(?,?,'person','raw',?,?,?,?,?,?)`, code, name, status, mobile, origin, prefix, remark, price)
	if err != nil {
		api.FailJSON(c, "DB_ERROR:"+err.Error())
		return true
	}
	...
	if id == 0 {
		_ = s.DB.QueryRow(`SELECT id FROM pur_supplier WHERE code=?`, code).Scan(&id)
	}
	api.OK(c, s.loadFarmer(id))
	return true
}

// ensureFarmerForWeigh 入厂建单：无 supplier_id 则按手机号匹配个人供应商，否则新建。
func (s *Services) ensureFarmerForWeigh(name, mobile, origin string) (int64, error) {
	...
	if mobile != "" {
		var id int64
		_ = s.DB.QueryRow(`SELECT id FROM pur_supplier WHERE COALESCE(is_deleted,0)=0 AND party_kind='person'
			AND status NOT IN ('eliminated','blacklist') AND mobile=? ORDER BY id LIMIT 1`, mobile).Scan(&id)
		if id > 0 {
			return id, nil
		}
	}
	code := fmt.Sprintf("P%s", time.Now().Format("060102150405"))
	prefix := "TR"
	res, err := s.DB.Exec(`INSERT INTO pur_supplier(code, name, party_kind, supplier_type, status, mobile, origin, trace_code_prefix, remark)
		VALUES(?,?,'person','raw','qualified',?,?,?,'')`, code, name, mobile, origin, prefix)
	if err != nil {
		return 0, err
	}
	id, _ := res.LastInsertId()
...
func (s *Services) updateFarmer(c *gin.Context) bool {
	...
	_, err := s.DB.Exec(`UPDATE pur_supplier SET name=COALESCE(NULLIF(?,''),name), mobile=COALESCE(NULLIF(?,''),mobile),
		origin=COALESCE(NULLIF(?,''),origin),
		trace_code_prefix=COALESCE(NULLIF(?,''),trace_code_prefix), status=COALESCE(NULLIF(?,''),status),
		remark=COALESCE(NULLIF(?,''),remark), default_unit_price=COALESCE(NULLIF(?,0),default_unit_price),
		updated_at=NOW() WHERE id=? AND COALESCE(is_deleted,0)=0 AND party_kind='person'`,
		strOr(body["name"]), strOr(body["mobile"]), strOr(body["origin"]),
		strOr(body["trace_code_prefix"]), status, strOr(body["remark"]), nullFloat(body["default_unit_price"]), id)
	if err != nil {
		api.FailJSON(c, "DB_ERROR:"+err.Error())
		return true
	}
...
func (s *Services) loadFarmer(id int64) gin.H {
	var code, name, mobile, origin, prefix, status, remark, created string
	var price float64
	err := s.DB.QueryRow(`SELECT code, name, COALESCE(mobile,''), COALESCE(origin,''),
		COALESCE(trace_code_prefix,''), status, COALESCE(remark,''), created_at, COALESCE(default_unit_price,0)
		FROM pur_supplier WHERE id=? AND COALESCE(is_deleted,0)=0 AND party_kind='person'`, id).
		Scan(&code, &name, &mobile, &origin, &prefix, &status, &remark, &created, &price)
	if err != nil {
		return gin.H{}
	}
	return gin.H{
...
func (s *Services) ensureInvBoxCodeImageColumn() {
	if s == nil || s.DB == nil {
		return
	}
	_, _ = s.DB.Exec(`ALTER TABLE inv_box_code ADD COLUMN IF NOT EXISTS image_url TEXT NOT NULL DEFAULT ''`)
}

// ---------- weigh tickets ----------

func (s *Services) handleWeighTickets(c *gin.Context, method, action string) bool {
	s.ensureInvBoxCodeImageColumn()
	...
	case action == "get":
		...
		s.attachWeighBoxProgress(m, id)
		s.attachWarehouseVerifyState(m, id)
		if claimsIsWarehouseOnly(middleware.Claims(c)) {
			m = maskWeighTicketForWarehouse(m)
			s.attachWeighBoxProgress(m, id) // mask 可能丢掉 boxes，再挂一次允许字段
			s.attachWarehouseVerifyState(m, id)
		}
		api.OK(c, m)
		return true
	...
	case strings.Contains(path, "/box-stock-in/complete") || strings.HasSuffix(action, "box-stock-in-complete"):
		return s.completeBoxStockInWeighTicket(c)
...
func (s *Services) listWeighTickets(c *gin.Context) bool {
	...
	args = append(args, pageSize, (pageNum-1)*pageSize)
	rows, err := s.DB.Query(`SELECT w.id, w.doc_no, w.supplier_id, COALESCE(f.name,''), w.channel, w.product_id,
		...
		COALESCE(p.name,''),
		COALESCE((SELECT s.status FROM pur_supplier_settlement s WHERE s.weigh_ticket_id=w.id AND COALESCE(s.status,'')!='void' ORDER BY s.id DESC LIMIT 1),''),
	...
	for rows.Next() {
		...
		row := gin.H{
			"id": id, "doc_no": docNo, "supplier_id": farmerID, "farmer_name": farmerName, "channel": channel,
...
// 入厂单：weighed=待入厂，gate_accepted=待入库，stocked=待结算。
func weighProcessPhase(kind, status, settleStatus string) string {
	...
	paid := settleStatus == "settle_paid" || settleStatus == "paid"
	switch st {
	case "weighed":
		if kind == "stockin" {
...
// gateWeighPhoto is one of the three required inbound photos.
type gateWeighPhoto struct {
	Key          string
	EvidenceType string
	URL          string
}

func photoMapString(m map[string]interface{}, keys ...string) string {
	for _, k := range keys {
		if u := strings.TrimSpace(strOr(m[k])); u != "" {
			return u
		}
	}
	return ""
...
func orderedImageURLs(body map[string]interface{}) []string {
	out := []string{}
	...
	if len(out) == 0 {
		if u := strings.TrimSpace(strOr(body["image_url"])); u != "" {
			out = append(out, u)
		}
	}
	return out
...
// parseGateWeighPhotos 入厂三张：材料过磅、磅显特写、近距离。
// App 传 photos 对象；管理端补单仍可只传 image_url(s)。
func parseGateWeighPhotos(body map[string]interface{}) ([]gateWeighPhoto, string) {
	byKey := map[string]string{}
	_, hasPhotosObj := body["photos"].(map[string]interface{})
	if raw, ok := body["photos"].(map[string]interface{}); ok {
		byKey["material"] = photoMapString(raw, "material", "weigh_material", "material_photo")
		byKey["scale_display"] = photoMapString(raw, "scale_display", "scale", "ocr", "ocr_scale")
		byKey["closeup"] = photoMapString(raw, "closeup", "site_photo", "near")
	}
	ordered := orderedImageURLs(body)
	fill := []string{"material", "scale_display", "closeup"}
	for i, k := range fill {
		if byKey[k] == "" && i < len(ordered) {
			byKey[k] = ordered[i]
		}
	}
	complete := byKey["material"] != "" && byKey["scale_display"] != "" && byKey["closeup"] != ""
	if complete {
		slots := []gateWeighPhoto{
			{Key: "material", EvidenceType: "weigh_material", URL: byKey["material"]},
			{Key: "scale_display", EvidenceType: "scale_display", URL: byKey["scale_display"]},
			{Key: "closeup", EvidenceType: "site_photo", URL: byKey["closeup"]},
		}
		for _, p := range slots {
			if !isValidSitePhotoURL(p.URL) {
				return nil, "EVIDENCE_INVALID:" + p.Key
			}
		}
		return slots, ""
	}
	if hasPhotosObj {
		for _, k := range fill {
			if byKey[k] == "" {
				return nil, "EVIDENCE_INCOMPLETE:" + k
			}
		}
	}
	urls := collectImageURLs(body)
	if len(urls) == 0 {
		return nil, "EVIDENCE_INCOMPLETE:site_photo"
	}
	out := make([]gateWeighPhoto, 0, len(urls))
	for _, u := range urls {
		if !isValidSitePhotoURL(u) {
			return nil, "EVIDENCE_INVALID:site_photo"
		}
		out = append(out, gateWeighPhoto{Key: "closeup", EvidenceType: "site_photo", URL: u})
	}
	return out, ""
}

func (s *Services) createWeighTicket(c *gin.Context) bool {
	setupCodes := []string{"organization", "plant", "warehouse_raw", "product", "purchase_gate_flow"}
	if s.purchaseFlowRequireWeigh("gate") {
		setupCodes = append(setupCodes, "weigh_variety")
	}
	if s.RequireSetupReady(c, setupCodes...) {
		return true
	}
	body := bindBody(c)
	...
	requireWeigh := s.purchaseFlowRequireWeigh(kind)
	...
	var photos []gateWeighPhoto
	if requireWeigh {
		var photoErr string
		photos, photoErr = parseGateWeighPhotos(body)
		if photoErr != "" {
			api.FailJSON(c, photoErr)
			return true
		}
	} else {
		// optional site photos
		if slots, err := parseGateWeighPhotos(body); err == "" {
			photos = slots
		} else if urls := collectImageURLs(body); len(urls) > 0 {
			for _, u := range urls {
				if isValidSitePhotoURL(u) {
					photos = append(photos, gateWeighPhoto{Key: "closeup", EvidenceType: "site_photo", URL: u})
				}
			}
		}
	}
	imgs := make([]string, 0, len(photos))
	imageURL := ""
	for _, p := range photos {
		imgs = append(imgs, p.URL)
		if p.Key == "closeup" || imageURL == "" {
			imageURL = p.URL
		}
	}
	...
	if arrivalID > 0 {
		...
		err := s.DB.QueryRow(`SELECT supplier_id, status, COALESCE(qc_result,''), COALESCE(grade,''), COALESCE(origin,''),
	...
	if farmerID <= 0 {
		...
		id, err := s.ensureFarmerForWeigh(partyName, partyMobile, origin)
		if err != nil || id <= 0 {
			api.FailJSON(c, "SUPPLIER_CREATE_FAILED")
			return true
		}
		farmerID = id
	}
	if farmerID > 0 {
		...
		err := s.DB.QueryRow(`SELECT name, COALESCE(origin,''), COALESCE(mobile,'') FROM pur_supplier
			WHERE id=? AND COALESCE(is_deleted,0)=0 AND status NOT IN ('eliminated','blacklist','frozen')`, farmerID).
	...
	if isTraceBatchInProgress(st) {
		...
		_ = s.DB.QueryRow(`SELECT COALESCE(supplier_id,0), COALESCE(product_id,0), COALESCE(variety,'')
		...
		if lockFarmer > 0 {
			...
			farmerID = lockFarmer
			if partyName == "" {
				_ = s.DB.QueryRow(`SELECT COALESCE(name,'') FROM pur_supplier WHERE id=?`, lockFarmer).Scan(&partyName)
			}
		}
		if lockProduct > 0 {
	...
	if kind == "gate" {
		...
		// 仓库：优先请求 warehouse_id → 品种绑定仓 → 按 warehouse_role / cold_store_type 查主数据
		coldStore = strings.ToLower(strOr(body["cold_store_type"]))
		if coldStore == "" {
			coldStore = s.inferColdStoreFromVarietyDB(variety)
		}
		warehouseID = 0
		if wid, ok := asInt64(body["warehouse_id"]); ok && wid > 0 {
			...
			var vWhID int64
			var vRole string
			_ = s.DB.QueryRow(`SELECT COALESCE(warehouse_id,0), COALESCE(warehouse_role,'') FROM pur_weigh_variety
				WHERE status='active' AND COALESCE(is_deleted,0)=0 AND (name=? OR code=?) ORDER BY id LIMIT 1`,
				variety, variety).Scan(&vWhID, &vRole)
			warehouseID = s.ResolveWarehouseForWeighVariety(vWhID, vRole, coldStore)
		}
		if warehouseID <= 0 {
			api.FailJSON(c, "WAREHOUSE_NOT_CONFIGURED")
			return true
		}
	} else {
		...
		if wid, ok := asInt64(body["warehouse_id"]); ok && wid > 0 {
			...
			warehouseID = s.ResolveWarehouseByStoreType(coldStore)
		}
		if warehouseID <= 0 {
			api.FailJSON(c, "WAREHOUSE_NOT_CONFIGURED")
			return true
	...
	res, err := s.DB.Exec(`INSERT INTO pur_weigh_ticket(doc_no, supplier_id, channel, ticket_template, product_id, variety,
	...
	for _, p := range photos {
		_, _ = s.addEvidence(c, "weigh_ticket", id, p.EvidenceType, p.URL, gin.H{"slot": p.Key})
	}
	...
	photoByKey := gin.H{}
	for _, p := range photos {
		photoByKey[p.Key] = p.URL
	}
	if strOr(photoByKey["material"]) != "" && strOr(photoByKey["scale_display"]) != "" {
		ticketPayload["photos"] = photoByKey
	}
...
func (s *Services) notifyWeighConfirmed(c *gin.Context, id int64, out gin.H) {
	...
	_, _ = s.DB.Exec(`DELETE FROM wf_task WHERE biz_type='weigh_ticket' AND biz_id=? AND (
		event_key IN ('purchase.weigh_confirmed','purchase.weigh_returned')
		OR (event_key='purchase.await_stockin' AND to_role='warehouse')
	)`, id)
	s.Notify.NotifyNext(c, notify.Event{
		...
		Payload: gin.H{
			...
			"status": "weighed", "weigh_ticket_id": id,
...
func (s *Services) applyVerifiedWeighStockIn(c *gin.Context, id int64, body map[string]interface{}) (out gin.H, errCode string, already bool) {
	...
	if s.Notify != nil {
		...
		if kind == "gate" {
			s.notifyWarehouseAwaitStockin(c, id, m)
			s.Notify.NotifyNext(c, notify.Event{
			...
			s.Notify.NotifyNext(c, notify.Event{
				...
				Payload: gin.H{
					"supplier_id": m["supplier_id"], "farmer_name": m["farmer_name"],
			...
			if settleID > 0 {
				payload := gin.H{
					...
					"supplier_id": m["supplier_id"], "farmer_name": m["farmer_name"], "unit_price": m["unit_price"],
					"settlement_id": settleID, "settle_breakdown": breakdown,
				}
				s.Notify.NotifyNext(c, notify.Event{
					Key: "purchase.gate_accepted", BizType: "weigh_ticket", BizID: id,
...
func (s *Services) boxStockInWeighTicket(c *gin.Context) bool {
	...
	if err := s.DB.QueryRow(`SELECT status, COALESCE(trace_code,'') FROM pur_weigh_ticket WHERE id=?`, id).
		Scan(&status, &trace); err != nil {
		api.FailJSON(c, "NOT_FOUND")
		return true
	}
	if status == "stocked" {
	...
	ok, msg, batchSum, boxCodes := s.doWeighStockInBatch(c, id, body, true)
	if !ok {
		api.FailJSON(c, msg)
		return true
	}
	...
	m := s.loadWeighTicket(id)
	s.writeAuditCtx(c, "weigh_ticket", id, "box_stock_in", "box_stockin_batch", nil, m)
	if cl := middleware.Claims(c); cl != nil {
		var tid int64
		_ = s.DB.QueryRow(`SELECT id FROM wf_ticket WHERE biz_type='weigh_ticket' AND biz_id=? AND status IN ('open','in_progress') ORDER BY id DESC LIMIT 1`, id).Scan(&tid)
		if tid > 0 {
			s.appendTicketLog(tid, "box_stock_in", cl.UserID, 0, fmt.Sprintf("batch boxes=%d sum=%.2f", len(boxCodes), batchSum))
		}
	}

	out := s.loadWeighTicket(id)
	s.attachWeighBoxProgress(out, id)
	out["batch_box_codes"] = boxCodes
	out["batch_sum_kg"] = batchSum
	out["completed"] = false
	api.OK(c, out)
	return true
}
...
func (s *Services) completeBoxStockInWeighTicket(c *gin.Context) bool {
	...
	// 允许完成前再交最后一批箱子
	if raw, ok := body["boxes"].([]interface{}); ok && len(raw) > 0 {
		ok2, msg, _, _ := s.doWeighStockInBatch(c, id, body, true)
		if !ok2 {
			api.FailJSON(c, msg)
			return true
		}
			...
			if sid > 0 && s.Notify != nil {
				s.Notify.NotifyNext(c, notify.Event{
					...
					Payload: gin.H{
						...
						"supplier_id": tm["supplier_id"], "farmer_name": tm["farmer_name"],
	...
	if cl := middleware.Claims(c); cl != nil {
		...
		_ = s.DB.QueryRow(`SELECT id FROM wf_ticket WHERE biz_type='weigh_ticket' AND biz_id=? AND status IN ('open','in_progress') ORDER BY id DESC LIMIT 1`, id).Scan(&tid)
		if tid > 0 {
			s.appendTicketLog(tid, "box_stock_in", cl.UserID, 0, "box_stockin_complete")
		}
	}
...
func (s *Services) ensureWeighIssued(c *gin.Context, id int64, body map[string]interface{}) string {
	...
	trace := batch
	farmerID, _ := asInt64(m["supplier_id"])
	...
	snap := nowSnap(map[string]interface{}{
		"gross_weight": gross, "deduct_rate": deductRate, "deduct_weight": deductWeight, "net_weight": net,
		"grade": grade, "trace_code": trace, "batch_no": batch, "supplier_id": farmerID, "bind": "batch_as_trace",
	})
	...
	if lotID <= 0 {
		_, err = s.DB.Exec(`INSERT INTO pur_trace_lot(trace_code, biz_date, batch_no, supplier_id, grade, arrival_id, weigh_ticket_id, channel, source_type, net_weight, payload_canonical, signature, status)
			VALUES(?,?,?,?,?,?,?,?,?,?,?,?,'open')`,
...
type inboundBoxLine struct {
	Weight    float64
	ImageURLs []string
}

func parseInboundBoxLines(body map[string]interface{}, requirePhotos bool) ([]inboundBoxLine, string) {
	if body == nil {
	...
	lines := make([]inboundBoxLine, 0, len(raw))
	for _, item := range raw {
		...
		collectURL(m["image_urls"])
		collectURL(m["image_url"])
		if requirePhotos && len(urls) == 0 {
			return nil, "BOX_PHOTO_REQUIRED"
		}
		lines = append(lines, inboundBoxLine{Weight: w, ImageURLs: urls})
	}
	return lines, ""
}

// deprecated shim for callers expecting weights only
func parseInboundBoxWeights(body map[string]interface{}) ([]float64, string) {
	lines, errMsg := parseInboundBoxLines(body, false)
	if errMsg != "" {
		return nil, errMsg
	}
...
func (s *Services) weighBoxedProgress(trace string) (count int, sum float64) {
	trace = strings.TrimSpace(trace)
	if trace == "" {
		return 0, 0
	}
	_ = s.DB.QueryRow(`SELECT COUNT(1), COALESCE(SUM(weight),0) FROM inv_box_code
		WHERE COALESCE(is_deleted,0)=0 AND UPPER(COALESCE(trace_code,''))=UPPER(?)`, trace).Scan(&count, &sum)
	return count, sum
}

func (s *Services) listWeighBoxes(trace string) []gin.H {
	trace = strings.TrimSpace(trace)
	out := []gin.H{}
	if trace == "" {
		return out
	}
	rows, err := s.DB.Query(`SELECT id, code, COALESCE(weight,0), COALESCE(image_url,''), COALESCE(status,''), COALESCE(created_at,'')
	...
	for rows.Next() {
		var id int64
		var code, img, st, created string
		var w float64
		_ = rows.Scan(&id, &code, &w, &img, &st, &created)
		imgs := []string{}
		if img != "" {
			imgs = append(imgs, img)
		}
		for _, e := range s.listEvidence("box_code", id) {
			if u := strOr(e["file_url"]); u != "" {
				dup := false
				for _, x := range imgs {
					if x == u {
						dup = true
		...
		out = append(out, gin.H{
			"id": id, "code": code, "weight": w, "image_url": img, "image_urls": imgs,
...
func (s *Services) attachWeighBoxProgress(out gin.H, id int64) {
	if out == nil {
		return
	}
...
func (s *Services) doWeighStockIn(id int64, body map[string]interface{}) (bool, string, float64) {
	...
	ok, msg, batchSum, _ := s.doWeighStockInBatch(nil, id, body, false)
	if !ok {
		return false, msg, 0
	}
	loss, errMsg := s.finalizeWeighBoxStockIn(id)
	if errMsg != "" {
...
func (s *Services) doWeighStockInBatch(c *gin.Context, id int64, body map[string]interface{}, requirePhotos bool) (ok bool, errCode string, batchSum float64, boxCodes []string) {
	...
	st := strOr(m["status"])
	if st == "stocked" {
		return false, "ALREADY_STOCKED", 0, nil
	}
	lines, errMsg := parseInboundBoxLines(body, requirePhotos)
	if errMsg != "" {
	...
	if wh <= 0 {
		if wid := asInt64Or0(m["warehouse_id"]); wid > 0 {
			wh = wid
		} else if cw := s.ResolveWarehouseByStoreType(strOr(m["cold_store_type"])); cw > 0 {
			wh = cw
		} else {
			return false, "WAREHOUSE_NOT_CONFIGURED", 0, nil
		}
	}

	boxCodes = make([]string, 0, len(lines))
	for i, ln := range lines {
		code := s.allocInboundBoxCode(trace, existCnt+i+1)
		img0 := ""
		if len(ln.ImageURLs) > 0 {
			img0 = ln.ImageURLs[0]
		}
		res, err := s.DB.Exec(`INSERT INTO inv_box_code(code, product_id, warehouse_id, batch_no, qty, weight, supplier_id, trace_code, origin, receive_date, source_type, status, current_process_id, current_step_id, image_url)
			VALUES(?,?,?,?,?,?,?,?,?,?,?,'open',?,?,?)`,
		...
		if err != nil {
			...
			res, err = s.DB.Exec(`INSERT INTO inv_box_code(code, product_id, warehouse_id, batch_no, qty, weight, supplier_id, trace_code, origin, receive_date, source_type, status, current_process_id, current_step_id, image_url)
				...
				code, productID, wh, bizDate, ln.Weight, ln.Weight, farmerID, trace, origin, bizDate, sourceType, procID, stepID, img0)
			if err != nil {
				return false, "BOX_CREATE_ERROR:" + err.Error(), 0, nil
			}
		}
		boxID, _ := res.LastInsertId()
		for _, u := range ln.ImageURLs {
			_, _ = s.addEvidence(c, "box_code", boxID, "box_reweigh", u, map[string]interface{}{
				"weigh_ticket_id": id, "box_code": code, "weight": ln.Weight,
			})
			_, _ = s.addEvidence(c, "weigh_ticket", id, "box_reweigh", u, map[string]interface{}{
				"box_id": boxID, "box_code": code, "weight": ln.Weight,
			})
		}
	...
	txnNo := fmt.Sprintf("ST-WT-%d-%d", id, time.Now().UnixNano()%1e9)
	docType := "purchase_in"
	...
	tres, err := s.DB.Exec(`INSERT INTO inv_stock_txn(doc_no, doc_type, biz_date, status, warehouse_id, remark) VALUES(?,?,?,'draft',?,?)`,
		txnNo, docType, bizDate, wh, fmt.Sprintf("weigh ticket #%d farmer=%d boxes=%d batch", id, farmerID, len(lines)))
	if err != nil {
		return false, "STOCK_TXN_ERROR:" + err.Error(), 0, nil
	}
	...
	for i, ln := range lines {
		_, _ = s.DB.Exec(`INSERT INTO inv_stock_txn_line(txn_id, line_no, product_id, qty, base_qty, direction, batch_no) VALUES(?,?,?,?,?,'in',?)`,
			tid, i+1, productID, ln.Weight, ln.Weight, bizDate)
	}
...
func (s *Services) finalizeWeighBoxStockIn(id int64) (inboundLoss float64, errCode string) {
	...
	if ticketNet > boxedSum {
		inboundLoss = ticketNet - boxedSum
	}
	productID := asInt64Or0(m["product_id"])
	wh := asInt64Or0(m["warehouse_id"])
	if wh <= 0 {
		wh = 1
	}
	bizDate := strOr(m["biz_date"])
	if inboundLoss > 0 {
		if err := s.writeInboundLossTxn(id, wh, productID, inboundLoss, bizDate); err != nil {
			return 0, "INBOUND_LOSS_ERROR:" + err.Error()
		}
	}
	firstBox := strOr(m["box_code"])
	if firstBox == "" {
...
func (s *Services) loadWeighTicket(id int64) gin.H {
	...
	err := s.DB.QueryRow(`SELECT w.doc_no, w.supplier_id, COALESCE(f.name,''), w.channel, COALESCE(w.ticket_template,''), w.product_id, w.variety,
...
func (s *Services) attachWeighProcessTrail(m gin.H, weighID int64) {
	...
	_ = s.DB.QueryRow(`SELECT COALESCE(status,''), COALESCE(doc_no,''), COALESCE(amount,0)
		FROM pur_supplier_settlement WHERE weigh_ticket_id=? AND COALESCE(status,'')!='void' ORDER BY id DESC LIMIT 1`, weighID).
	...
	m["process_phase"] = weighProcessPhase(strOr(m["receive_kind"]), strOr(m["status"]), settleStatus)
	m["process_logs"] = logs
	m["currency"] = "CNY"
...
func (s *Services) handleFarmerSettlements(c *gin.Context, method, action string) bool {
	...
	if action == "list" || method == "GET" {
		pageNum, pageSize := sqlutil.Page(c)
		var total int
		_ = s.DB.QueryRow(`SELECT COUNT(1) FROM pur_supplier_settlement`).Scan(&total)
		rows, err := s.DB.Query(`SELECT s.id, s.doc_no, s.supplier_id, COALESCE(f.name,''), s.weigh_ticket_id, s.biz_date,
			...
			FROM pur_supplier_settlement s LEFT JOIN pur_supplier f ON f.id=s.supplier_id
	...
	if action == "create" || method == "POST" {
		...
		var bizDate string
		if wtID > 0 {
			_ = s.DB.QueryRow(`SELECT supplier_id, net_weight, biz_date FROM pur_weigh_ticket WHERE id=?`, wtID).Scan(&farmerID, &net, &bizDate)
		}
		if farmerID == 0 {
			farmerID, _ = asInt64(body["supplier_id"])
			net, _ = asFloat(body["net_weight"])
		...
		id, _ := res.LastInsertId()
		out := gin.H{"id": id, "doc_no": docNo, "amount": amount, "status": "settle_pending"}
		if payID, _ := s.maybeCreatePaymentOrderForSettlement(id); payID > 0 {
			out["payment_order_id"] = payID
		}
		api.OK(c, out)
		return true
	}
...
func (s *Services) payFarmerSettlement(c *gin.Context) bool {
	...
	var status string
	var wtID int64
	if err := s.DB.QueryRow(`SELECT status, COALESCE(weigh_ticket_id,0) FROM pur_supplier_settlement WHERE id=?`, id).Scan(&status, &wtID); err != nil {
		api.FailJSON(c, "NOT_FOUND")
		return true
	}
	if wtID > 0 {
	...
	if s.blockManualPayIfOnlinePending(c, id) {
		return true
	}
	s.ensureFinanceCashColumns()
	...
	_, err := s.DB.Exec(`UPDATE pur_supplier_settlement SET status='settle_paid', transfer_no=?, paid_at=NOW(), pay_evidence_url=?,
...
func (s *Services) summaryFarmerSettlements(c *gin.Context) bool {
	...
	q := `SELECT supplier_id, COALESCE(f.name,''), SUM(net_weight), SUM(amount), COUNT(1)
		FROM pur_supplier_settlement s LEFT JOIN pur_supplier f ON f.id=s.supplier_id WHERE 1=1`
	args := []interface{}{}
	if bizDate != "" {
		q += ` AND s.biz_date=?`
	...
	for rows.Next() {
		...
		list = append(list, gin.H{"supplier_id": fid, "farmer_name": name, "net_weight": net, "amount": amt, "count": cnt})
	}
...
func (s *Services) traceTimeline(c *gin.Context, code string) bool {
	...
	var net float64
	err := s.DB.QueryRow(`SELECT id, trace_code, biz_date, batch_no, supplier_id, COALESCE(grade,''), COALESCE(arrival_id,0), COALESCE(weigh_ticket_id,0),
	...
	if err == nil {
		lot = gin.H{"id": lotID, "trace_code": trace, "biz_date": bizDate, "batch_no": batch, "supplier_id": farmerID,
	...
	if len(weighIDs) > 0 {
		for _, wid := range weighIDs {
			srows, qerr := s.DB.Query(`SELECT id, doc_no, amount, status, COALESCE(transfer_no,''), COALESCE(paid_at,'') FROM pur_supplier_settlement WHERE weigh_ticket_id=?`, wid)
	...
	auditRows, _ := s.DB.Query(`SELECT action, reason, created_at, COALESCE(actor_user_id,0) FROM biz_audit_log
		WHERE (biz_type='weigh_ticket' AND biz_id=?) OR (biz_type='inbound_arrival' AND biz_id=?) OR (biz_type='farmer_settlement' AND biz_id IN (SELECT id FROM pur_supplier_settlement WHERE weigh_ticket_id=?))
