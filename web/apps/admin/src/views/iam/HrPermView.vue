...
const filteredPerms = computed(() => {
  if (!permDomainFilter.value) return permissions.value
  return permissions.value.filter((p) => p.domain === permDomainFilter.value)
})
...
async function loadCore() {
  loading.value = true
  try {
    const [u, r, e, s, g, ov, onb, off, p] = await Promise.all([
      iamApi.users(),
      iamApi.roles(),
      hrApi.employees(),
      iamApi.sessions(),
    ...
    sessions.value = ((s.data as { list?: Row[] })?.list) || []
    groups.value = ((g.data as { list?: Row[] })?.list) || []
    overview.value = (ov.data as Row) || {}
    onboards.value = ((onb.data as { list?: Row[] })?.list) || []
    offboards.value = ((off.data as { list?: Row[] })?.list) || []
            ...
            <el-row :gutter="16">
              <el-col :span="12">
                ...
                <el-checkbox-group v-model="roleWhIds">
                  <el-checkbox v-for="w in warehouses" :key="String(w.id)" :label="Number(w.id)">
                    {{ w.name }} ({{ w.code }})
                  </el-checkbox>
                </el-checkbox-group>
              ...
              <el-col :span="12">
                <h3 class="sub">权限码授权</h3>
                <el-select v-model="permDomainFilter" clearable placeholder="按域筛选" style="width:180px;margin-bottom:8px">
                  <el-option v-for="d in permDomains" :key="d" :label="d" :value="d" />
                </el-select>
                <el-checkbox-group v-model="rolePermIds" class="perm-list">
                  <el-checkbox v-for="p in filteredPerms.slice(0, 80)" :key="String(p.id)" :label="Number(p.id)">
                    {{ p.code }}
                  </el-checkbox>
                </el-checkbox-group>
                <p v-if="filteredPerms.length > 80" class="muted">仅显示前 80 条，可用域筛选缩小范围</p>
                <el-button type="primary" style="margin-top:10px" @click="saveRolePerms">保存权限码</el-button>
              </el-col>
            </el-row>
...
.perm-list { display: flex; flex-direction: column; max-height: 280px; overflow: auto; gap: 2px; }
